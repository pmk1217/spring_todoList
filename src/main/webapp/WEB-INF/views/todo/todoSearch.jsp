...
<div style="width:60%; margin: 0 auto;">
				...
				<c:otherwise>
					 ...
					 <div id="onetable">
						...
						<div class="content" style="text-align: right;">
			       		 ...
			       		 <a href="${contextPath}/todo/removeTodo.do?todoseq=${todo.todoseq}" class="btn">삭제</a>
						</div>
						</div> 
					  </c:forEach>
					</c:otherwise>     
					...
					<div class="page">
        <c:if test="${pageNo > 1}">
            <a href="?userid=${loginInfo.userid}&amp;searchType=${param.searchType}&amp;keyword=${param.keyword}&amp;pageNo=${pageNo - 1}">이전</a>
        </c:if>
        <c:forEach var="page" begin="1" end="${totalPages}">
            <c:choose>
                ...
                <c:otherwise>
                    <a class="pagebtn" href="?userid=${loginInfo.userid}&amp;searchType=${param.searchType}&amp;keyword=${param.keyword}&amp;pageNo=${page}">${page}</a>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        <c:if test="${pageNo < totalPages}">
            <a href="?userid=${loginInfo.userid}&amp;searchType=${param.searchType}&amp;keyword=${param.keyword}&amp;pageNo=${pageNo + 1}">다음</a>
        </c:if>
    </div>
